...
void CTAFrame::set_user_env_var_tab_title()
{
    // Set the title of user env. var. tab according to UAC
    // ConsentPromptBehavior flag
    char * pValue;
    pValue = getenv ("USERNAME");
    if (pValue!=NULL)
    {
        std::string new_title("User environment variables");
        new_title += std::string(" (") + pValue + ")";
        notebook_1_->SetPageText(0, new_title.c_str());
    }
}
